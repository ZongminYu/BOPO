...
class TSPModel(nn.Module):
    ...
    def forward(self, state):
        ...
        sols_num = state.BATCH_IDX.size(1)
        problem_size = state.problem_size

        if state.current_node is None:
            if self.model_params['start_node'] == 'same':
                selected = torch.zeros(batch_size, sols_num, dtype=torch.int64)
            elif self.model_params['start_node'] == 'random':
                selected = torch.randint(0, problem_size, (batch_size, sols_num), dtype=torch.int64)
            elif self.model_params['start_node'] == 'pomo':
                selected = torch.arange(0, problem_size, dtype=torch.int64).unsqueeze(0).expand(batch_size, -1)
            else:
                raise NotImplementedError
            prob = torch.ones(size=(batch_size, sols_num))

            encoded_first_node = _get_encoding(self.encoded_nodes, selected)
            # shape: (batch, B, embedding)
...
def _get_encoding(encoded_nodes, node_index_to_pick):
    ...
    # node_index_to_pick.shape: (batch, B)

    batch_size = node_index_to_pick.size(0)
    sols_num = node_index_to_pick.size(1)
    embedding_dim = encoded_nodes.size(2)

    gathering_index = node_index_to_pick[:, :, None].expand(batch_size, sols_num, embedding_dim)
    # shape: (batch, B, embedding)
...
class TSP_Decoder(nn.Module):
    ...
    def set_kv(self, encoded_nodes):
        ...
        # shape: (batch, head_num, B, qkv_dim)
        self.single_head_key = encoded_nodes.transpose(1, 2)
        # shape: (batch, embedding, problem)

    def set_q1(self, encoded_q1):
        # encoded_q.shape: (batch, n, embedding)  # n can be 1 or B
        head_num = self.model_params['head_num']
    ...
    def forward(self, encoded_last_node, ninf_mask):
        ...
        # shape: (batch, B, head_num*qkv_dim)
