...
def cluster_edges(num_j: int, num_m: int, machines: torch.Tensor,
                  device: str = 'cpu', job_first=True):
    """
    Make the cluster of jobs and machines as an edge list.
    ...
    for j in range(num_j):
        for m in range(num_m-1):
            edges_j.append((j * num_m + m, j * num_m + m+1))
            edges_j.append((j * num_m + m+1, j * num_m + m))
    ...
    for m in range(num_m):
        ...
        for src_i in range(len(machine) - 1):
            for dst_i in range(src_i + 1, len(machine)):
                #
                edges_m.append((machine[src_i].item(), machine[dst_i].item()))
                edges_m.append((machine[dst_i].item(), machine[src_i].item()))
    edges_m = torch.tensor(edges_m, dtype=torch.long, device=device)

    return (edges_j, edges_m) if job_first else (edges_m, edges_j)
...
def load_data_v2(path, device: str = 'cpu', sep: str = ' '):
    """
    Load a JSP instance from path and return a PyTorch Data object.
    Note that the instance is loaded as a DiGraph.
    ...
    # Make the disjunctive graph
    edges_j, edges_m = cluster_edges(num_j, num_m, machines, device=device, job_first=True)
...
def load_dataset(path: str = './dataset/',
    ...
    c_path = os.path.join(path, f"cached_{shape}.pt"
                                if shape else 
                                ('cached_split.pt'
                                if split else 'cached.pt'))
    # c_path = os.path.join(path, 'cached_test_split.pt') 
    if use_cached and os.path.exists(c_path):
        ...
        for file in os.listdir(path):
            if file.startswith('.') or file.startswith('cached') or \
                ...
                continue
            if split:
                instances.append(load_data_v2(os.path.join(path, file),
